...
export class IDLProblemDetector {
  ...
  _detectProblems() {
    ...
    this.previousProblems =
      this.previousProblems.length === 0
        ? Object.keys(this.problems)
        : this.previousProblems.concat(Object.keys(this.problems));
    ...
    // gather TODO statements
    this._detectTODOStatements();
  }
  ...
  private _detectTODOStatements() {
    const todos = this.idl.manager.todos;
    const files = Object.keys(todos);
    for (let i = 0; i < files.length; i++) {
      const key = files[i];

      // get the TODO statemetns
      const fileTodo = todos[key];

      // save if needed
      if (fileTodo.length > 0) {
        // check if we have a promblem entry for our file
        if (!this.problems[key]) {
          this.problems[key] = [];
        }

        // add it
        this.problems[key] = this.problems[key].concat(fileTodo);
      }
    }
  }
