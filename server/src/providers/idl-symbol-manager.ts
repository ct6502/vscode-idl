...
// get our globby code
// const glob = require('glob-fs')({ gitignore: false }); // file searching
...
import { extractToDo } from '../parsing/extract-todo';
import { resolveCompletionItemKind } from '../parsing/resolve-routine-extras';
import { IDL } from './idl';
...
export class IDLSymbolManager {
  ...
  workspaceFiles: { [key: string]: { [key: string]: boolean } } = {}; // track files in the workspace so we properly clean up on removal. nested object for faster random access
    ...
    documentSymbols: moize(
      async (uri: string, updating = false): Promise<IDLDocumentSymbol[]> =>
        new Promise((resolve, reject) => {
          try {
            // get the strings we are processing without comments and extra spaces
            const text = this.idl.files.getFileString(uri);

            // extract all of our TODO statements
            // get the normal strings for our file including the comments
            const textWithComments = this.idl.files.strings[uri];
            this.todos[uri] = extractToDo(textWithComments);

            // extract symbols - routines and variables
      ...
      switch (true) {
        ...
        case query.isMethod && query.equalBefore:
          matches = fuzzysort.go(
            query.searchName,
            Object.keys(this.quickSearchLookup.functionMethods),
            searchOptions
          );
          break;
        // procedure method
        case query.isMethod && !query.equalBefore:
          matches = fuzzysort.go(
            query.searchName,
            Object.keys(this.quickSearchLookup.procedureMethods),
            searchOptions
          );
          break;
        // functions
        case query.equalBefore || query.isFunction:
          matches = fuzzysort.go(
            query.searchName,
            Object.keys(this.quickSearchLookup.functions),
            searchOptions
          );
          break;
        ...
        default:
          matches = fuzzysort.go(
            query.searchName,
            Object.keys(this.quickSearchLookup.procedures),
            searchOptions
          );
      }
  ...
  completion(
    ...
    items = items.length === 0 ? add : items.concat(add);
  ...
  async update(uri: string): Promise<DocumentSymbol[]> {
    ...
    // get updated
    return this.get.documentSymbols(uri);
  }
  ...
  async removeWorkspaces(folders: WorkspaceFolder[]) {
    ...
    for (let i = 0; i < folders.length; i++) {
      ...
      const flag = folder.uri in this.workspaceFiles;
      if (!flag) {
        continue;
      }

      // get the files
      const files = Object.keys(this.workspaceFiles[folder.uri]);

      // process each file
      for (let z = 0; z < files.length; z++) {
        // remove all symbls
        await this.remove(files[z]);
      }

      // clean up
      delete this.workspaceFiles[folder.uri];
    }
  }
  ...
  async indexWorkspaces(folders: WorkspaceFolder[]) {
    ...
    try {
      ...
      folders.forEach(folder => {
        ...
        // search for files
        const files: string[] = glob.sync('**/*.pro', { cwd: folderPath });

        // init array for files in our folder, populated below
        this.workspaceFiles[folder.uri] = {};

        // process each file
        files.forEach(file => {
          // get the URI as a string
          const uriStr = URI.file(folderPath + path.sep + file).toString();
          this.workspaceFiles[folder.uri][uriStr] = true;
  ...
  private _removeSymbols(uri: string, symbols: DocumentSymbol[]) {
    ...
    symbols.forEach(symbol => {
      ...
      // remove source information for this symbol
      delete this.routineSymbolLookup[key];
      delete this.routineCompletionLookup[key];

      // clean up functions
      delete this.quickLookup.functions[key];
      delete this.quickSearchLookup.functions[key];

      // clean up function methods
      delete this.quickLookup.functionMethods[key];
      delete this.quickSearchLookup.functionMethods[key];

      // clean up procedures
      delete this.quickLookup.procedures[key];
      delete this.quickSearchLookup.procedures[key];

      // clean up procedure methods
      delete this.quickLookup.procedureMethods[key];
      delete this.quickSearchLookup.procedureMethods[key];
